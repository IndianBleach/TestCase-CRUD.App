...
using CRUDApp.Core.Enums;
using CRUDApp.Core.Interfaces;
using CRUDApp.Infrastructure.Data;
using Microsoft.EntityFrameworkCore;


namespace CRUDApp.Infrastructure.Services.OrderService
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<string> CreateAsync(CreateOrderDto dtoModel)
        {
            ...
            if (getProvider != null)
            {
                if (!string.IsNullOrWhiteSpace(dtoModel.Number) &&
                    dtoModel.Date != null &&
                    dtoModel.ProviderId != null)
                {
        ...
        public async Task<bool> DeleteAsync(DeleteOrderDto dtoModel)
        {
            Order? getOrder = await _appContext.Orders
                .Include(x => x.OrderItems)
                .Include(x => x.Provider)
